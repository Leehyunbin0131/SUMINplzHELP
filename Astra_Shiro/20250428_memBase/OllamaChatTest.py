# OllamaChatTest.py (중요도 평가 제거 + STT 콜백 수정 버전)
import requests
import json
import time
import logging
...
# import re # 중요도 평가 제거로 더 이상 필요하지 않음
...
try:
    from system_prompts import (
        ...
        get_astra_siro_identity_context 
    )
except ModuleNotFoundError:
...
# 로깅 설정 - config 파일 로드 성공 후 수행
logging.basicConfig(
    level=getattr(logging, config.LOG_LEVEL, 'INFO'), # 기본값 INFO 추가
...
if not REALTIME_STT_AVAILABLE:
     # 로거 설정 후 경고 메시지 로깅
     logger.warning("RealtimeSTT 모듈을 찾을 수 없습니다. 텍스트 입력으로만 동작합니다.")
...
class VoiceLLMAssistant:
    def __init__(self, ollama_host=config.OLLAMA_HOST, model=config.DEFAULT_MODEL,
        ...
        logger.info("단기 기억 버퍼 (최대 10턴) 초기화 완료")

        try:
            self.setup_stt_recorder()
        except Exception as e:
            logger.error(f"STT 레코더 설정 실패: {e}")
    ...
    def setup_mem0_for_ltm(self):
        ...
        logger.info("LTM 저장용 Memory 시스템 설정 중 (ChromaDB 사용)...")
        mem0_config = {
            "vector_store": {
        ...
        try:
            ...
            logger.info(f"LTM 저장용 Memory 시스템 설정 완료 (Vector Store: ChromaDB at '{config.CHROMA_PATH}', Embedder: Ollama)")
            return memory_instance
        except Exception as e:
            logger.error(f"LTM 저장용 Memory 시스템 설정 실패: {e}")
    ...
    def test_ollama_connection(self, server_url, server_name="Ollama"):
        ...
        try:
            ...
            logger.info(f"{server_name} 서버에 성공적으로 연결됨 ({server_url}): {response.json()}")
        except requests.exceptions.RequestException as e:
            logger.error(f"{server_url}에 있는 {server_name} 서버에 연결할 수 없음: {e}")
    ...
    def setup_stt_recorder(self):
        ...
        if REALTIME_STT_AVAILABLE:
            logger.info("RealtimeSTT 레코더 설정 중...")
            recorder_config = {
                ...
                "on_transcription_start": lambda audio_data: logger.info("오디오 변환 중..."),
                # --- 수정 끝 ---
            ...
            logger.info("RealtimeSTT 레코더 설정 완료")
        else:
            logger.info("텍스트 입력 모드로 레코더 설정 (RealtimeSTT 없음)")
            self.recorder = AudioToTextRecorder()

    def _on_recording_start(self): logger.info("🎤 녹음 시작됨")
    def _on_recording_stop(self): logger.info("🛑 녹음 중지됨, 변환 처리 중...")
    ...
    def save_to_ltm(self, conversation_text):
        ...
        **수정됨:** 중요도 평가 없이 모든 대화 내용을 LTM에 저장합니다.
        ...
        try:
            ...
            logger.info(f"대화 내용을 LTM에 저장했습니다: {conversation_text[:100]}...")
        except Exception as e:
            logger.error(f"LTM 저장 중 예상치 못한 오류: {e}", exc_info=True)


    def process_voice_input(self):
        """음성 또는 텍스트 입력을 처리하고, 결과를 얻어 메인 LLM에 전송합니다."""
        if self.is_processing:
            logger.warning("이미 요청을 처리 중입니다. 잠시 기다려주세요")
            return
    ...
    def send_to_llm(self, text):
        ...
        try:
            ...
            logger.debug(f"검색된 LTM 컨텍스트:\n{ltm_context}")
        except Exception as e:
            logger.error(f"LTM 검색 중 오류 발생: {e}", exc_info=True)
            ltm_context = "장기 기억 검색 중 오류 발생."
        try:
            dynamic_identity_context = get_astra_siro_identity_context()
            logger.debug(f"사용될 동적 정체성 컨텍스트:\n{dynamic_identity_context[:300]}...")
        except Exception as e:
             logger.error(f"동적 정체성 컨텍스트 생성 중 오류: {e}", exc_info=True)
        ...
        try:
            ...
            logger.debug(f"메인 LLM에 전송될 최종 프롬프트:\n{prompt_with_context}")
        except KeyError as e:
            logger.error(f"프롬프트 템플릿 포맷팅 오류: 누락된 키 - {e}")
            print(f"\n❌ 오류: 프롬프트를 구성할 수 없습니다.")
            return
        except Exception as e:
             logger.error(f"프롬프트 구성 중 예상치 못한 오류: {e}", exc_info=True)
             print(f"\n❌ 오류: 프롬프트를 구성할 수 없습니다.")
             return
        payload = {
            "model": self.model,
        ...
        try:
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    try:
                        ...
                        logger.warning(f"응답 스트림 JSON 디코딩 오류 (무시): {decoded_line}")
                    except Exception as e:
                        logger.error(f"응답 스트림 처리 중 오류: {e}", exc_info=True)
            print("\n")
            if text and full_response.strip():
                ...
                logger.info("현재 대화를 STM에 추가했습니다.")
                self.save_to_ltm(interaction_to_save)
        except requests.exceptions.Timeout:
             logger.error(f"Ollama API 호출 시간 초과 ({self.ollama_url})")
             print(f"\n❌ 오류: LLM 응답 시간이 초과되었습니다.")
        except requests.exceptions.RequestException as e:
            logger.error(f"Ollama API 호출 오류: {e}")
            print(f"\n❌ 오류: LLM 서버({self.ollama_url}) 응답을 받을 수 없습니다 ({e}).")
        except Exception as e:
            logger.error(f"LLM 응답 처리 중 예상치 못한 오류: {e}", exc_info=True)
            print(f"\n❌ 처리 중 오류 발생: {e}")
        ...
        try:
            ...
            if hasattr(self, 'recorder') and hasattr(self.recorder, 'shutdown') and callable(self.recorder.shutdown):
                 try:
                     ...
                     logger.info("STT 레코더가 성공적으로 종료되었습니다.")
                 except Exception as e:
                     logger.error(f"STT 레코더 종료 중 오류 발생: {e}")
...
if __name__ == "__main__":
    ...
    if args.debug:
        ...
        logger.info("명령줄 인자로 디버그 모드 활성화됨 (--debug)")
    else:
    ...
    logger.info(f"로깅 레벨 설정됨: {logging.getLevelName(log_level)}")
